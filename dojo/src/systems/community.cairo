...
pub trait ICommunity<TState> {
    ...
    fn create_quiz_party(ref self: TState, name: ByteArray, description: ByteArray, start: u64) -> QuizParty; //@description: Create a quiz event (admin)
...
pub mod community {
    ...
    pub mod Errors {
        ...
        pub const INVALID_PARTY: felt252        = 'COMMUNITY: Invalid party';
        pub const PARTY_IS_CLOSED: felt252      = 'COMMUNITY: Party is closed';
        pub const INVALID_NAME: felt252         = 'COMMUNITY: Invalid name';
        pub const QUESTION_IS_CLOSED: felt252   = 'COMMUNITY: Question is closed';
    ...
    impl CommunityImpl of super::ICommunity<ContractState> {
        ...
        fn create_quiz_party(ref self: ContractState, name: ByteArray, description: ByteArray, start: u64) -> QuizParty {
            let mut store: Store = StoreTrait::new(self.world_default());
            self._assert_caller_is_admin();
            (QuizPartyTrait::create_quiz_party(ref store, name, description, start))
        }
        fn edit_quiz_party(ref self: ContractState, party_id: u32, name: ByteArray, description: ByteArray, start: u64) -> QuizParty {
            let mut store: Store = StoreTrait::new(self.world_default());
            self._assert_caller_is_admin();
            (QuizPartyTrait::edit_quiz_party(ref store, party_id, name, description, start))
        }
        fn close_quiz_party(ref self: ContractState, party_id: u32) -> QuizParty {
            let mut store: Store = StoreTrait::new(self.world_default());
            self._assert_caller_is_admin();
            (QuizPartyTrait::close_quiz_party(ref store, party_id))
        }
        ...
        fn close_quiz_question(ref self: ContractState, party_id: u32, question_id: u32, answer_number: u8) -> QuizQuestion {
            ...
            (QuizQuestionTrait::close_quiz_question(ref store, party_id, question_id, answer_number, vrf))
        }
        fn answer_quiz_question(ref self: ContractState, party_id: u32, question_id: u32, answer_number: u8) -> QuizAnswer {
            let mut store: Store = StoreTrait::new(self.world_default());
            (QuizQuestionTrait::answer_quiz_question(ref store, party_id, question_id, answer_number))
        }
