...
fn setup_uninitialized(fee_amount: u256) -> (IWorldDispatcher, IDuelistTokenDispatcher) {
    ...
    world.grant_owner(dojo::utils::bytearray_hash(@"origami_token"), lords.contract_address);
    ...
    let duelists_call_data: Span<felt252> = array![
        ...
        (fee_amount * CONST::ETH_TO_WEI).try_into().unwrap(), // 100 Lords
    ].span();
...
fn setup(fee_amount: u256) -> (IWorldDispatcher, IDuelistTokenDispatcher) {
