...
mod tests {
    ...
    fn test_invalid_challenged_not_your_duelist() {
        ...
        let _duel_id: u128 = tester::execute_create_challenge(actions, HIGH, OTHER(), MESSAGE_1, TABLE_ID, 0, 0);
    }
    ...
    //     let _duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), HIGH, MESSAGE_1, TABLE_ID, 0, 0);
    // }
    ...
    fn test_invalid_challenged_self_duelist() {
        let (_world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        let _duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), OWNER(), MESSAGE_1, TABLE_ID, 0, 0);
    }
    ...
    fn test_invalid_challenged_self_address() {
        let (_world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        let _duel_id: u128 = tester::execute_create_challenge(actions, LITTLE_BOY(), LITTLE_BOY(), MESSAGE_1, TABLE_ID, 0, 0);
    }
    ...
    fn test_invalid_challenged_zero() {
        let (_world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        let _duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), ZERO(), MESSAGE_1, TABLE_ID, 0, 0);
    }
    ...
    fn test_challenge_exists() {
        let (_world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        tester::execute_create_challenge(actions, OWNER(), OTHER(), MESSAGE_1,TABLE_ID, 0, 0);
        tester::execute_create_challenge(actions, OWNER(), OTHER(), MESSAGE_1, TABLE_ID, 0, 0);
    }
    ...
    fn test_challenge_exists_from_challenged() {
        let (_world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        tester::execute_create_challenge(actions, OWNER(), OTHER(), MESSAGE_1, TABLE_ID, 0, 0);
        tester::execute_create_challenge(actions, OTHER(), OWNER(), MESSAGE_1, TABLE_ID, 0, 0);
    }
    ...
    fn test_challenge_to_address() {
        ...
        let duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), BIG_BOY(), MESSAGE_1, TABLE_ID, 0, 0);
    ...
    fn test_challenge_to_duelist() {
        let (world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        let duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), OTHER(), MESSAGE_1, TABLE_ID, 0, 0);
        let ch = tester::get_ChallengeEntity(world, duel_id);
        assert(ch.state == ChallengeState::Awaiting, 'state');
        assert(ch.address_a == OWNER(), 'challenged');
    ...
    fn test_challenge_expire_ok() {
        ...
        let timestamp = tester::get_block_timestamp();
        let duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), OTHER(), MESSAGE_1, TABLE_ID, 0, expire_hours);
        let ch = tester::get_ChallengeEntity(world, duel_id);
        assert(ch.timestamp_start == timestamp, 'timestamp_start');
    ...
    fn test_challenge_address_pact() {
        let (_world, actions, _admin, _lords, _minter) = tester::setup_world(flags::ACTIONS | flags::APPROVE);
        assert(actions.get_pact(TABLE_ID, ID(OWNER()), ID(OTHER())) == 0, 'get_pact_0_1');
        assert(actions.get_pact(TABLE_ID, ID(OTHER()), ID(OWNER())) == 0, 'get_pact_0_2');
        assert(actions.has_pact(TABLE_ID, ID(OWNER()), ID(OTHER())) == false, 'has_pact_0_1');
        assert(actions.has_pact(TABLE_ID, ID(OTHER()), ID(OWNER())) == false, 'has_pact_0_2');
        let duel_id: u128 = tester::execute_create_challenge(actions, OWNER(), OTHER(), MESSAGE_1, TABLE_ID, 0, 0);
        assert(actions.get_pact(TABLE_ID, ID(OWNER()), ID(OTHER())) == duel_id, 'get_pact_1_1');
        assert(actions.get_pact(TABLE_ID, ID(OTHER()), ID(OWNER())) == duel_id, 'get_pact_1_2');
        assert(actions.has_pact(TABLE_ID, ID(OWNER()), ID(OTHER())) == true, 'has_pact_1_1');
        assert(actions.has_pact(TABLE_ID, ID(OTHER()), ID(OWNER())) == true, 'has_pact_1_2');
    }
    ...
    fn test_challenge_reply_invalid() {
        ...
        tester::execute_reply_challenge(actions, A, duel_id + 1, true);
    }
    ...
    fn test_challenge_reply_twice() {
        ...
        let B: ContractAddress = OTHER();
        let duel_id: u128 = tester::execute_create_challenge(actions, A, B, MESSAGE_1, TABLE_ID, 0, 48);
        let _ch = tester::get_ChallengeEntity(world, duel_id);
    ...
    fn test_challenge_reply_expired_id() {
        ...
        assert(actions.has_pact(ch.table_id, ID(B), ID(A)) == false, 'has_pact_no');
    ...
    fn test_challenge_reply_expired_address() {
        ...
        assert(ch.duelist_id_b == 0, 'challenged_id'); // challenged an address, id is empty
        assert(actions.has_pact(ch.table_id, ID(A), ID(B)) == true, 'has_pact_addr_true');
        assert(actions.has_pact(ch.table_id, ID(ID_A), ID(ID_B)) == false, 'has_pact_id_false');
        
        let (_block_number, timestamp) = tester::elapse_timestamp(timestamp::from_date(1, 0, 1));
        let new_state: ChallengeState = tester::execute_reply_challenge_ID(actions, B, ID(ID_B), duel_id, false);
        assert(new_state == ChallengeState::Expired, 'expired');
        let ch = tester::get_ChallengeEntity(world, duel_id);
        assert(ch.state == new_state, 'state');
        assert(ch.round_number == 0, 'round_number');
        ...
        assert(ch.timestamp_start < timestamp, 'timestamp_start');
        assert(ch.timestamp_end == timestamp, 'timestamp_end');
        assert(actions.has_pact(ch.table_id, ID(A), ID(B)) == false, 'has_pact_addr_false');
        assert(actions.has_pact(ch.table_id, ID(ID_A), ID(ID_B)) == false, 'has_pact_id_false_still');
    ...
    fn test_challenge_owner_accept_self() {
        ...
        tester::elapse_timestamp(timestamp::from_days(1));
        tester::execute_reply_challenge(actions, A, duel_id, true);
    }
    ...
    fn test_challenge_owner_cancel() {
        ...
        assert(actions.has_pact(ch.table_id, ID(A), ID(B)) == false, 'has_pact_no');
    ...
    fn test_challenge_impersonator() {
        ...
        tester::execute_reply_challenge(actions, BUMMER(), duel_id, false);
    }
