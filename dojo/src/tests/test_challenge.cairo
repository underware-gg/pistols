...
mod tests {
    ...
    fn test_invalid_challenged_not_your_duelist() {
        ...
        let _duel_id: u128 = tester::execute_create_duel(@sys.duels, FAKE_OWNER_OF_1(), OTHER(), PREMISE_1, SEASON_1_TABLE(), 0);
    }
    ...
    fn test_invalid_challenged_self() {
        let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::APPROVE);
        let _duel_id: u128 = tester::execute_create_duel(@sys.duels, OWNER(), OWNER(), PREMISE_1, SEASON_1_TABLE(), 0);
    }
    ...
    fn test_invalid_challenged_zero() {
        let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::APPROVE);
        let _duel_id: u128 = tester::execute_create_duel(@sys.duels, OWNER(), ZERO(), PREMISE_1, SEASON_1_TABLE(), 0);
    }
    ...
    fn test_challenge_pact_exists() {
        let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::APPROVE);
        tester::execute_create_duel_ID(@sys.duels, OWNER(), ID(OWNER()), OTHER(), PREMISE_1,SEASON_1_TABLE(), 0);
        tester::execute_create_duel_ID(@sys.duels, OWNER(), OWNED_BY_OWNER(), OTHER(), PREMISE_1, SEASON_1_TABLE(), 0);
    }
    ...
    fn test_challenge_pact_exists_from_challenged() {
        let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::APPROVE);
        tester::execute_create_duel(@sys.duels, OWNER(), OTHER(), PREMISE_1, SEASON_1_TABLE(), 0);
        tester::execute_create_duel(@sys.duels, OTHER(), OWNER(), PREMISE_1, SEASON_1_TABLE(), 0);
    }
    ...
    fn test_challenge_duelist_busy() {
        let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::APPROVE);
        tester::execute_create_duel(@sys.duels, OWNER(), OTHER(), PREMISE_1,SEASON_1_TABLE(), 0);
        tester::execute_create_duel(@sys.duels, OWNER(), BUMMER(), PREMISE_1,SEASON_1_TABLE(), 0);
    }
    ...
    fn test_challenge_duelist_busy_reply() {
        let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::APPROVE);
        let _duel_id: u128 = tester::execute_create_duel(@sys.duels, OWNER(), OTHER(), PREMISE_1,SEASON_1_TABLE(), 0);
        let duel_id: u128 = tester::execute_create_duel(@sys.duels, BUMMER(), OWNER(), PREMISE_1,SEASON_1_TABLE(), 0);
    ...
    fn test_challenge_ok() {
        ...
        let duel_id: u128 = tester::execute_create_duel(@sys.duels, OWNER(), OTHER(), PREMISE_1, SEASON_1_TABLE(), 0);
    ...
    fn test_challenge_expire_ok() {
        ...
        let timestamp: u64 = tester::get_block_timestamp();
        let duel_id: u128 = tester::execute_create_duel(@sys.duels, OWNER(), OTHER(), PREMISE_1, SEASON_1_TABLE(), expire_hours);
        let ch = tester::get_ChallengeValue(sys.world, duel_id);
        assert_eq!(ch.timestamp_start, timestamp, "timestamp_start");
    ...
    fn test_challenge_reply_invalid() {
        ...
        let A = OWNER();
        let B = OTHER();
        let duel_id: u128 = tester::execute_create_duel(@sys.duels, A, B, PREMISE_1, SEASON_1_TABLE(), 48);
