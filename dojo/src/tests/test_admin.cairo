...
mod tests {
    ...
    fn test_initialize_defaults() {
        let (world, _system, admin, _lords, _ierc20) = tester::setup_world(false, false);
    ...
    fn test_set_owner_defaults() {
        let (_world, _system, admin, _lords, _ierc20) = tester::setup_world(false, false);
        tester::execute_admin_initialize(admin, OWNER(), ZERO(), ZERO(), ZERO());
        let config: Config = admin.get_config();
    ...
    fn test_initialize_table() {
        let (_world, _system, admin, lords, _ierc20) = tester::setup_world(false, false);
    ...
    #[available_gas(1_000_000_000)]
    fn test_set_table() {
        let (world, _system, admin, lords, _ierc20) = tester::setup_world(false, false);
        // not initialized
        tester::execute_admin_initialize(admin, OWNER(), ZERO(), ZERO(), ZERO());
        let table: TableConfig = admin.get_table(tables::LORDS);
    ...
    fn test_set_table_count() {
        let (_world, _system, admin, _lords, _ierc20) = tester::setup_world(true, false);
    ...
    fn test_enable_table_count() {
        let (_world, _system, admin, lords, _ierc20) = tester::setup_world(true, false);
