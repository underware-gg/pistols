...
mod tests {
    ...
    fn test_calc_lethal_bonus() {
        ...
        assert(utils::calc_lethal_bonus(villain.score) == chances::LETHAL_BONUS_VILLAIN, 'level_100');
        villain.score.level_villain = 90;
        assert(utils::calc_lethal_bonus(villain.score) < chances::LETHAL_BONUS_VILLAIN, 'level_90');
        villain.score.level_villain = 50;
        assert(utils::calc_lethal_bonus(villain.score) == chances::LETHAL_BONUS_VILLAIN/2, 'level_50');
        villain.score.level_villain = 10;
        assert(utils::calc_lethal_bonus(villain.score) > 0, 'level_10');
        villain.score.level_villain = 0;
        assert(utils::calc_lethal_bonus(villain.score) == 0, 'level_0');
        //
        // Clamp to total_duels
        villain.score.level_villain = 100;
        villain.score.total_duels = 9;
        assert(utils::calc_lethal_bonus(villain.score) < chances::LETHAL_BONUS_VILLAIN, 'total_duels_9');
        villain.score.total_duels = 5;
        assert(utils::calc_lethal_bonus(villain.score) == chances::LETHAL_BONUS_VILLAIN/2, 'total_duels_5');
        villain.score.total_duels = 1;
        assert(utils::calc_lethal_bonus(villain.score) > 0, 'total_duels_1');
        villain.score.total_duels = 0;
        assert(utils::calc_lethal_bonus(villain.score) == 0, 'total_duels_0');
        //
        // not for lords
        let lord: Duelist = _lord(100);
        assert(utils::calc_lethal_bonus(lord.score) == 0, 'no_lords');
        //
        // tricksters get half
        let mut trickster: Duelist = _trickster(100);
        assert(utils::calc_lethal_bonus(trickster.score) == chances::LETHAL_BONUS_TRICKSTER, 'trickster_level_100');
        trickster.score.level_trickster = 50;   
        assert(utils::calc_lethal_bonus(trickster.score) == chances::LETHAL_BONUS_TRICKSTER/2, 'trickster_level_50');
        trickster.score.level_trickster = 10;
        assert(utils::calc_lethal_bonus(trickster.score) > 0, 'trickster_level_10');
        trickster.score.level_trickster = 0;
        assert(utils::calc_lethal_bonus(trickster.score) == 0, 'trickster_level_0');
    }
    ...
    fn test_calc_hit_chances() {
        ...
        let chances_0: u8 = utils::calc_hit_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        assert(chances_0 > 0, 'chances_0 > 0');
        //
        // 50% bonus
        villain.score.level_villain = 50;
        let chances_50: u8 = utils::calc_hit_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        assert(chances_50 > 0, 'chances_50 > 0');
        assert(chances_50 > chances_0, 'chances_50 > chances_0');
        //
        // 100% bonus
        villain.score.level_villain = 100;
        let chances_100: u8 = utils::calc_hit_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        assert(chances_100 > 0, 'chances_100 > 0');
        assert(chances_100 > chances_50, 'chances_100 > chances_50');
        //
        // TRICKSTER: no bonus
        let mut trickster: Duelist = _trickster(0);
        let trickster_0: u8 = utils::calc_hit_chances(trickster.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
        assert(trickster_0 > 0, 'trickster_0 > 0');
        assert(trickster_0 == chances_0, 'trickster_0 ==');
        //
        // 50% bonus
        trickster.score.level_trickster = 50;
        let trickster_50: u8 = utils::calc_hit_chances(trickster.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
        assert(trickster_50 > 0, 'trickster_50 > 0');
        assert(trickster_50 > trickster_0, 'trickster_50 > trickster_0');
        assert(trickster_50 < chances_50, 'trickster_50 <');
        //
        // 100% bonus
        trickster.score.level_trickster = 100;
        let trickster_100: u8 = utils::calc_hit_chances(trickster.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
        assert(trickster_100 > 0, 'trickster_100 > 0');
        assert(trickster_100 > trickster_50, 'trickster_100 > trickster_50');
        assert(trickster_100 < chances_100, 'trickster_100 <');
        //
        // NO BONUS FOR LORDS
        let mut lord: Duelist = _lord(0);
        let lord_0: u8 = utils::calc_hit_chances(lord.score, lord.score, Action::Paces5, constants::FULL_HEALTH);
        assert(lord_0 > 0, 'lord_0 > 0');
        //
        // 50%
        lord.score.level_lord = 50;
        let lord_50: u8 = utils::calc_hit_chances(lord.score, lord.score, Action::Paces5, constants::FULL_HEALTH);
    ...
    fn test_trickster_penalty() {
        ...
        let chances_v_l: u8 = utils::calc_hit_chances(villain.score, lord.score, Action::Paces5, constants::FULL_HEALTH);
        let chances_v_t: u8 = utils::calc_hit_chances(villain.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
    ...
    fn test_calc_lethal_chances() {
        ...
        // no bonus
        let mut villain: Duelist = _villain(0);
        let chances_0: u8 = utils::calc_lethal_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        assert(chances_0 > 0, 'chances_0 > 0');
        //
        // 50% bonus
        villain.score.level_villain = 50;
        let chances_50: u8 = utils::calc_lethal_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        assert(chances_50 > 0, 'chances_50 > 0');
        assert(chances_50 > chances_0, 'chances_50 > chances_0');
        //
        // 100% bonus
        villain.score.level_villain = 100;
        let chances_100: u8 = utils::calc_lethal_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        assert(chances_100 > 0, 'chances_100 > 0');
        assert(chances_100 > chances_50, 'chances_100 > chances_50');
        //
        // TRICKSTER: no bonus
        let mut trickster: Duelist = _trickster(0);
        let trickster_0: u8 = utils::calc_lethal_chances(trickster.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
        assert(trickster_0 > 0, 'trickster_0 > 0');
        assert(trickster_0 == chances_0, 'trickster_0 ==');
        //
        // 50% bonus
        trickster.score.level_trickster = 50;
        let trickster_50: u8 = utils::calc_lethal_chances(trickster.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
        assert(trickster_50 > 0, 'trickster_50 > 0');
        assert(trickster_50 > trickster_0, 'trickster_50 > trickster_0');
        assert(trickster_50 < chances_50, 'trickster_50 <');
        //
        // 100% bonus
        trickster.score.level_trickster = 100;
        let trickster_100: u8 = utils::calc_lethal_chances(trickster.score, trickster.score, Action::Paces5, constants::FULL_HEALTH);
        assert(trickster_100 > 0, 'trickster_100 > 0');
        assert(trickster_100 > trickster_50, 'trickster_100 > trickster_50');
        assert(trickster_100 < chances_100, 'trickster_100 <');
        //
        // NO BONUS FOR LORDS
        let mut lord: Duelist = _lord(0);
        let lord_0: u8 = utils::calc_lethal_chances(lord.score, lord.score, Action::Paces5, constants::FULL_HEALTH);
        assert(lord_0 > 0, 'lord_0 > 0');
        //
        // 50%
        lord.score.level_lord = 50;
        let lord_50: u8 = utils::calc_lethal_chances(lord.score, lord.score, Action::Paces5, constants::FULL_HEALTH);
        assert(lord_0 == lord_50, 'lord_0 == lord_50');
    }
    ...
    fn test_calc_hit_lethal_sync() {
        ...
        // no bonus
        let mut villain: Duelist = _villain(0);
        let hit_chances_0: u8 = utils::calc_hit_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        let lethal_chances_0: u8 = utils::calc_lethal_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        ...
        // 50% bonus
        villain.score.level_villain = 50;
        let hit_chances_50: u8 = utils::calc_hit_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        let lethal_chances_50: u8 = utils::calc_lethal_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        ...
        // 100% bonus
        villain.score.level_villain = 100;
        let hit_chances_100: u8 = utils::calc_hit_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
        let lethal_chances_100: u8 = utils::calc_lethal_chances(villain.score, villain.score, Action::Paces5, constants::FULL_HEALTH);
