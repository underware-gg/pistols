...
mod tests {
    ...
    fn test_register_ChallengeTable() {
        ...
        tester::execute_register_duelist(system, OWNER(), name, 1, "1");

        let duelist: Duelist = tester::get_Duelist(world, OWNER());
        assert(duelist.name == name, 'duelist name');
        assert(duelist.profile_pic_type == 1, 'duelist profile_pic_type');
        assert(duelist.profile_pic_uri == "1", 'duelist profile_pic_uri');
    ...
    fn test_register_challenged() {
        ...
        tester::execute_register_duelist(system, OWNER(), player1_name, 1, "1");
        tester::execute_register_duelist(system, OTHER(), player2_name, 1, "2");
