...
use pistols::systems::{
    community::{ICommunityDispatcherTrait},
};
use pistols::models::{
    events::{
        // PlayerBookmarkEvent,
...
fn test_emit_player_bookmark() {
    let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::COMMUNITY);
    // set bookmark
    tester::impersonate(OWNER());
    tester::drop_dojo_events(@sys);
    tester::assert_no_dojo_events_left(@sys);
    sys.community.emit_player_bookmark(BUMMER(), 0, true);
    tester::assert_event_bookmark(@sys, OWNER(), BUMMER(), 0, true);
    tester::assert_no_dojo_events_left(@sys);
    // unset bookmark
    tester::impersonate(OWNER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_bookmark(BUMMER(), 0, false);
    tester::assert_event_bookmark(@sys, OWNER(), BUMMER(), 0, false);
    tester::assert_no_dojo_events_left(@sys);
    // another one...
    tester::impersonate(OTHER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_bookmark(TREASURY(), 123, true);
...
fn test_emit_player_social_link() {
    ...
    tester::impersonate(OWNER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_social_link(SocialPlatform::Discord, BUMMER(), "username", "1234567890", "_avatar_1");
    tester::assert_event_social_link(@sys, BUMMER(), SocialPlatform::Discord, "username", "1234567890", "_avatar_1");
    tester::assert_no_dojo_events_left(@sys);
    // unset link (player)
    tester::impersonate(BUMMER());
    tester::drop_dojo_events(@sys);
    sys.community.clear_player_social_link(SocialPlatform::Discord);
    ...
    tester::impersonate(OTHER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_social_link(SocialPlatform::Telegram, BUMMER(), "USERNAME", "3333", "_avatar_2");
...
fn test_emit_player_social_link_not_admin() {
    let mut sys: TestSystems = tester::setup_world(FLAGS::GAME | FLAGS::COMMUNITY);
    tester::impersonate(OTHER());
    sys.community.emit_player_social_link(SocialPlatform::Discord, OTHER(), "username", "1234567890", "_avatar_3");
}
...
fn test_emit_player_setting() {
    ...
    tester::impersonate(OTHER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_setting(PlayerSetting::OptOutNotifications(SocialPlatform::Undefined), PlayerSettingValue::Boolean(true));
    ...
    tester::impersonate(OTHER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_setting(PlayerSetting::OptOutNotifications(SocialPlatform::Discord), PlayerSettingValue::Boolean(true));
    ...
    tester::impersonate(OTHER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_setting(PlayerSetting::OptOutNotifications(SocialPlatform::Discord), PlayerSettingValue::Undefined);
    ...
    tester::impersonate(OTHER());
    tester::drop_dojo_events(@sys);
    sys.community.emit_player_setting(PlayerSetting::Undefined, PlayerSettingValue::Boolean(true));
    tester::assert_no_dojo_events_left(@sys);
}
