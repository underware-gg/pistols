...
export class ProgressDialogManager {
  ...
  constructor(scene: THREE.Scene, duelistAPosition: THREE.Vector3, duelistBPosition: THREE.Vector3) {
    ...
    this.dialogA = new ProgressDialogMesh(duelistAPosition, true)
    this.dialogB = new ProgressDialogMesh(duelistBPosition, false)
  }

  public setData(duelistAName: string, duelistBName: string, isYouA: boolean) {
    this.isDialogAYou = isYouA

    this.dialogA.setDuelistData(duelistAName, isYouA)
    this.dialogB.setDuelistData(duelistBName, !isYouA)
  }

  public showDialogs() {
    new TWEEN.Tween((this.dialogA.children[0] as HTMLMesh).material)
      .to({ opacity: 1 }, FADE_ANIMATION_DURATION)
      .easing(TWEEN.Easing.Quadratic.InOut)
      .start()
    new TWEEN.Tween((this.dialogB.children[0] as HTMLMesh).material)
      .to({ opacity: 1 }, FADE_ANIMATION_DURATION)
      .easing(TWEEN.Easing.Quadratic.InOut)
      .start()
  }
  ...
  public hideDialogs() {
    new TWEEN.Tween((this.dialogA.children[0] as HTMLMesh).material)
      .to({ opacity: 0 }, FADE_ANIMATION_DURATION)
      .easing(TWEEN.Easing.Quadratic.InOut)
      .start()
    new TWEEN.Tween((this.dialogB.children[0] as HTMLMesh).material)
      .to({ opacity: 0 }, FADE_ANIMATION_DURATION)
  ...
  public updateDialogState(stageA: any, stageB: any, onClick: any) {
    const { stateA, stateB } = this.findCurrentDuelStates(stageA, stageB)
    ...
    if (this.lastDuelStateA != stateA) {
      this.dialogA.updateElementData(dialogAProps.isThinkingBubble, messageA, this.isDialogAYou ? onClick : null)
    }
    if (this.lastDuelStateB != stateB) {
      this.dialogB.updateElementData(dialogBProps.isThinkingBubble, messageB, this.isDialogAYou ? null : onClick)
    }
  ...
  public showDialogEnd(healthA: number, healthB: number) {
    ...
    this.dialogA.updateElementData(false, messageA, null)
    this.dialogB.updateElementData(false, messageB, null)
  }

}


export class ProgressDialogMesh extends THREE.Group {
  ...
  constructor(position: THREE.Vector3, isLeft: boolean) {
    super()
  ...
  public updatePosition(newPositionX: number) {
  ...
  public updateElementData(isThinkingBubble: boolean, message: string, onButtonClick: () => void) {
  ...
  private updateElement() {
    ...
    const bubbleImage = this.isThinkingBubble ? 'url(/textures/cards/card_back.png)' : 'url(/textures/cards/card_back.png)'
    ...
    // this.element.style.backgroundImage = bubbleImage
    this.element.style.backgroundColor = this.isThinkingBubble ? 'burlywood' : 'black'

    if (this.isThinkingBubble) {
      const button = this.element.querySelector('.dialog-button') as HTMLButtonElement
      if (button) {
    ...
    this.mesh.dispose()
    this.mesh = new HTMLMesh(this.element)
    this.add(this.mesh)
